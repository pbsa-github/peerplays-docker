...
: ${PEERPLAYS_DOCKER_SOURCE="https://gitlab.com/PBSA/PeerplaysIO/tools-libs/peerplays-docker"}
...
tput clear
	...
	echo
	echo "Commands:
#---WITNESS NODE AS SERVICE---#
	
	iaas                         - Builds, Installs and Starts the Peerplays Blockchain as a Service
	iaab                         - Builds, Installs Peerplays Blockchain (Manual Start)

#---WITNESS NODE AS DOCKER CONTAINER---#

	iaac                         - installs and starts ${DOCKER_NAME} docker container
	start                        - starts the ${DOCKER_NAME} container
	stop                         - stops ${DOCKER_NAME} container
	kill                         - force stop ${DOCKER_NAME} container (in event of ${DOCKER_NAME} container hanging indefinitely)
	restart                      - restarts ${DOCKER_NAME} container
	status                       - show status of ${DOCKER_NAME} container

#---Common commands---#
	logs                         - shows the logs related to Peerplays Blockchain
	uninstall                    - uninstalls the peerplays blockchain installation
	replay                       - starts replay of the installed Peerplays Blockchain installation	
...
start() {
	...
	seed_exists
	if [[ $? == 0 ]]; then
		if sudo docker start $DOCKER_NAME;
		   then 
...
if ! sudo systemctl stop peerplays.service
then 
	msg bold red "Peerplays witness node stop failed!!!!!"
	exit
else
	if ! sudo systemctl list-unit-files --type service|grep peerplays-replay.service
	then
		msg bold blue "Creating Peerplays Blockchain Replay Service"
		echo -e "[Unit]" > "$INSTALL_ABS_DIR"/peerplays.service
		{ echo -e "Description=Peerplays Witness Node\n"; echo -e "[Service]"; echo -e "User=$(whoami)"; echo -e "WorkingDirectory=$INSTALL_ABS_DIR"; echo -e "ExecStart=$INSTALL_ABS_DIR/src/peerplays/programs/witness_node/witness_node --replay-blockchain"; echo -e "Restart=always\n"; echo -e "[Install]"; echo -e "WantedBy=mult-user.target"; } >> "$INSTALL_ABS_DIR"/peerplays-replay.service
	...
	if [ "$(systemctl is-active peerplays-replay.service)" = "active" ]
	then 
...
then
   if ! sudo systemctl list-unit-files --type service|grep peerplays
   then 
	 peerplays_docker_replay
   else
	 peerplays_replay
...
if [ "$(systemctl is-active peerplays.service)" = "active" ]
then
  ...
  exit
elif [ "$(systemctl is-active peerplays-replay.service)" = "active" ]
then
  msg bold green "Tailing Peerplays service logs" 
  #WITNESS_SCRIPT=`systemctl status peerplays|grep ExecStart |awk -F\= '{print $2}'|sed -e 's/[^ ]* *$//'`
  #WITNESS_SCRIPT_DIR=`systemctl status peerplays|grep witness|grep peerplays|awk '{print $2}'|xargs dirname`
...
#
son_network_exists() {
    networkcount=$(sudo docker network ls | grep son | wc -l)
...
msg bold blue "Creating Peerplays Blockchain Service"
	...
	{ echo -e "Description=Peerplays Witness Node\n"; echo -e "[Service]"; echo -e "User=$(whoami)"; echo -e "WorkingDirectory=$DIRECTORY"; echo -e "ExecStart=$DIRECTORY/src/peerplays/programs/witness_node/witness_node"; echo -e "Restart=always\n"; echo -e "[Install]"; echo -e "WantedBy=mult-user.target"; } >> "$DIRECTORY"/peerplays.service
...
if [ "$(grep ^NAME /etc/os-release | awk -F\" '{print $2}')" = "Ubuntu" ] && [ "$(grep ^VERSION_ID /etc/os-release | awk -F\" '{print $2}')" = "18.04"  ]
then 
...
read -r -p "Enter peerplays blockchain branch/tag to use for this installation [Press Enter for default: beatrice]: " BRANCH
...
uninstall_peerplays(){
set_variables
	if sudo systemctl list-unit-files --type service|grep peerplays.service
	then 
	...
	elif sudo systemctl list-unit-files --type service|grep peerplays-replay.service
	then
...
case $1 in
	build)
		msg bold magenta "You may want to use '$0 install' for a binary image instead, it's faster."
		build "${@:2}"
		;;
	iaas)
